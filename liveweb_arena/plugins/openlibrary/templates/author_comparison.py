...
- Large entity pool: C(70,2)×3 metrics×2 result counts = 14,490 variants
...
class AuthorMetric(Enum):
    ...
    ALREADY_READ = ("already_read_count", "total already-read count")
...
- Tie rule: alphabetically earlier author name wins"""
    ...
    def _sum_metric(
        ...
        for work in top_n:
            ...
            if value is None:
                return GroundTruthResult.fail(
                    f"Missing '{metric}' for work '{title}' by '{author_name}'"
                )
            total += int(value)
